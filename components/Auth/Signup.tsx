...
const Signup: React.FC<SignupProps> = ({ onSwitchToLogin }) => {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Auto-login logic might be needed depending on Supabase config (confirm email vs auto confirm)
      // Usually signup returns a session if email confirmation is off, otherwise user needs to check email.
  ...
  return (
    <div className="w-full max-w-md p-8 bg-white/90 backdrop-blur-md rounded-2xl shadow-xl border border-white/20">
      <div className="text-center mb-8">
        <h2 className="text-3xl font-bold text-slate-800 mb-2">Inscription</h2>
        <p className="text-slate-500">Rejoignez-nous pour créer des cours incroyables</p>
      </div>
